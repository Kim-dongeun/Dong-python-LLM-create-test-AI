...
def main():
    ...
    if cl.get_session_state('user_id') is None or 'mode' not in st.session_state:
        st.session_state.mode = None
        login_signup()
    else:
        st.sidebar.title(f"환영합니다, {st.session_state.user['name']}님!" if st.session_state.user else "환영합니다!")
        
        if st.sidebar.button("로그아웃", key="logout_button", use_container_width=True):
            cl.logout()
            st.rerun()
...
def mode_select():
    ...
    admin_image_path = r"image/admin_image.jpg"
...
def create_questions():
    ...
    if subject:
        ...
        for i in range(num_subtopics):
            with st.expander(f"소주제 {i+1} 설정", expanded=False):
                ...
                if subtopic_name and subtopic_file:
                    question_types = {}
                    for qt_kr, qt_en in question_types_dict.items():
...
def create_room():
    ...
    # 질문 목록 표시
    all_questions = [q for subject in selected_subjects for subtopic in categorized_questions[subject] for q in categorized_questions[subject][subtopic]]
    questions_by_type = cl.categorize_questions_by_type(all_questions)
    ...
    # 이전 total_questions 값을 저장
    prev_total_questions = cl.get_session_state('prev_total_questions', 0)
    total_questions = col2.number_input("총 문제 개수", max_value=30, value=prev_total_questions)

    # 총 문제 개수가 변경되었는지 확인
    if total_questions != prev_total_questions:
        # 랜덤 선택 적용
        if total_questions != 0:
            random_pick = cl.select_random_questions(categorized_questions, selected_subjects, selected_subtopics, total_questions)
            questions_to_show = random_pick
        else:
            if cl.get_session_state('selected_category') == "전체":
                questions_to_show = all_questions
            else:
                questions_to_show = questions_by_type.get(cl.get_session_state('selected_category'), [])
        
        # 새로운 questions_to_show로 세션 상태 업데이트
        cl.set_session_state('questions_to_show', questions_to_show)
        
        # 이전 total_questions 값 업데이트
        cl.set_session_state('prev_total_questions', total_questions)
    else:
        # 총 문제 개수가 변경되지 않았다면, 저장된 questions_to_show 사용
        questions_to_show = cl.get_session_state('questions_to_show', all_questions)
    ...
    with col2:
        if st.button("전체 선택", use_container_width=True):
            ...
            st.success("현재 표시된 모든 문제가 선택되었습니다.")
            st.rerun()
    with col3:
        if st.button("선택 해제", use_container_width=True):
            for q in questions_to_show:
                cl.set_session_state(f'global_selected_questions_{q["id"]}', False)
            st.success("현재 표시된 모든 문제의 선택이 해제되었습니다.")
            st.rerun()

    # 질문 목록 표시
    questions_per_row = 3
    with st.container(height=700):
        rows = [st.columns(questions_per_row) for _ in
                range((len(questions_to_show) + questions_per_row - 1) // questions_per_row)]
    ...
    # 선택된 문제 저장
    if apply:
        selected_questions = [q for q in all_questions if cl.get_session_state(f'global_selected_questions_{q["id"]}', False)]
...
def show_room_results(room_id):
    st.header("시험 결과")
    if st.button("❌ 닫기", key=f"close_results_{room_id}", use_container_width=True):
    ...
    if df is not None:
        ...
        # JavaScript를 사용하여 브라우저 창 크기를 가져오는 코드
        st.markdown(
            """
            <script>
                // JavaScript로 창의 크기를 측정하여 Streamlit에 전달
                function updateWindowSize() {
                    var width = window.innerWidth;
                    var height = window.innerHeight;
                    // Streamlit에 값 전달
                    window.parent.postMessage(
                        {type: 'windowSize', width: width, height: height}, '*');
                }
                updateWindowSize();
        ...
        # 세션 상태에서 창 크기 정보 받기
        if 'window_width' in st.session_state and 'window_height' in st.session_state:
        ...
        chart = alt.Chart(hist_df).mark_bar().encode(
            ...
            width=window_width,  # 그래프 너비
            height=window_height  # 그래프 높이
        ).configure_axis(
        ...
        with st.container(border=True):
            ...
            <div class="horizontal-metrics">
                ...
                <div class="metric-container">
                    <div class="metric-label">총 참가자 수</div>
                    <div class="metric-value">{total_participants}명</div>
                </div>
                <div class="metric-container">
                    <div class="metric-label">평균 정답 수</div>
                    <div class="metric-value">{avg_correct:.1f}개</div>
                </div>
                <div class="metric-container">
                    <div class="metric-label">문제 수</div>
                    <div class="metric-value">{avg_total:.0f}개</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if not rooms:
        ...
        for room in rooms:
            ...
            with st.expander(f"**{status_emoji} {room['room_name']}**", expanded=False):
                ...
                if st.button("입장", key=f"enter_room_{room['id']}"):
    ...
    if st.button("문제 생성", use_container_width=True):
        if subject and subtopics_data:
            with st.spinner("문제를 생성하는 중입니다..."):
                ...
                st.write(result)
                st.write(cl.get_session_state('personal_questions'))
                st.write(subject)
                st.write(subtopics_data)
                if result:
                    st.success("문제 생성이 완료되었습니다!")
...
def show_personal_questions():
            ...
            for qt, questions in question_types.items():
                for i, question in enumerate(questions):
                    ...
                    with st.container(border=True):
                        ...
                        with col1:
                            st.markdown(f"**문제 {question_index}.**")
                            cl.display_personal_question(question, qt)
                            
                            # 사용자 답변 입력
                            if qt == "multiple-choice":
                                choices = question[1] if isinstance(question, tuple) else question.get('choices', [])
                                choices = ["선택하지 않음"] + choices
    ...
    if st.button("채점하기", use_container_width=True):
        if 'personal_user_answers' in st.session_state:
            user_id = cl.get_session_state('user_id')
            ...
            st.error("제출된 답변이 없습니다. 문제를 풀고 제출해주세요.")
...
def show_question_solving_session():
    ...
    for i, question in enumerate(session['questions']):
        with st.expander(f"문제 {i+1}", expanded=True):
